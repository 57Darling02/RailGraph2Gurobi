...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch run main analyze stage for case configs produced by case_library_builder.py."
    )
    parser.add_argument(
        "--config-root",
...
def main() -> None:
    ...
    for idx, config_path in enumerate(configs, start=1):
        ...
        try:
            ...
            if not loaded.analyze.enable_metrics and not loaded.analyze.enable_plot:
                record["status"] = "skipped"
            else:
                if not loaded.analyze.adjusted_timetable_path.exists():
